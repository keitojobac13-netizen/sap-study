...
export async function generateMetadata({ params }: {
  ...
  return {
    ...buildModuleMetadata(getModule(module, 'en'), 'en'),
    alternates: alternatesFor('en', (l) => modulePath(l, module)),
  };
}

export default async function ModulePageEn({ params }: {
  params: Promise<{ module: string }>;
}) {
  const { module } = await params;
  if (!isModuleKey(module)) notFound();
  return <ModuleHub moduleKey={module} lang="en" />;
}
