...
export const moreTerms: GlossaryTerm[] = [
  // ─── FI ──────────────────────────────────────────────────────
  {
    ...
    ja: {
      term: '税コード',
      reading: 'ぜいこーど',
      definition: '消費税などの税率と、税額を転記する勘定を決める2桁のコード。FTXPで国ごとの税計算手順にもとづいて定義し、伝票明細に指定することで税額が自動計算される。日本では標準税率10%と軽減税率8%、課税仕入と課税売上、対象外・非課税などを区別するため、複数の税コードを使い分ける。SDやMMでは税コードも条件テクニックで自動決定される。',
    },
    en: {
      term: 'Tax Code',
  ...
  },
  {
    ...
    ja: {
      term: '無償品',
      reading: 'むしょうひん',
      definition: '一定数量を購入した得意先に、商品を無償で提供する仕組み。注文数量の一部を無償とする「内数」と、注文数量に追加して無償で渡す「外数」がある。外数の場合は明細カテゴリTANNの無償明細が自動で追加される。条件はVBN1で登録し、条件テクニックで決定される。無償でも原価は発生するため、収益性分析での扱いを確認しておく必要がある。',
    },
    en: {
      term: 'Free Goods',
  ...
  },
  {
    ...
    ja: {
      term: '品目決定',
      reading: 'ひんもくけってい',
      definition: '受注入力時に、入力された品目を別の品目に自動で置き換える機能。キャンペーン期間中だけ販促用パッケージの品目に差し替える、JANコードで入力された品目を社内の品目コードに変換する、といった用途がある。VB11で条件レコードを登録し、条件テクニックで決定する。置き換えを確認画面で選ばせるか自動で行うかも設定できる。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '更新タスク',
      reading: 'こうしんたすく',
      definition: 'CALL FUNCTION ... IN UPDATE TASKで呼び出した更新汎用モジュールを、COMMIT WORKの時点で更新ワークプロセスがまとめて実行する仕組み。ユーザーの画面処理とデータベース更新を切り離すことで応答を速くし、SAP LUWの整合性を保つ。重要な更新はV1、統計などの二次的な更新はV2で処理される。更新が失敗した記録はSM13で確認する。',
    },
    en: {
      term: 'Update Task',
  ...
  // ─── Basis ───────────────────────────────────────────────────
  {
    ...
    ja: {
      ...
      definition: '他のシステムを呼び出すための接続情報で、SM59で定義する。タイプ3はABAPシステムへの接続、TはTCP/IPで外部プログラムへ、GやHはHTTPでの接続を表す。接続先にユーザーとパスワードを保存すると呼出し側の誰でもその権限で相手先を操作できてしまうため、権限の強いユーザーを登録しないことや、信頼関係（トラステッドRFC）の利用を検討する。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: 'クライアントコピー',
      reading: 'くらいあんとこぴー',
      definition: 'あるクライアントのデータを別のクライアントへ複製する処理。同じシステム内のコピー（SCCL）、別システムからのリモートコピー（SCC9）、エクスポート／インポート（SCC8）がある。何をコピーするかはプロファイルで選び、カスタマイジングだけ、ユーザーマスタだけ、全データなどを指定する。テスト用クライアントの作成や、本番に近いデータでの検証環境の準備に使う。',
    },
    en: {
      term: 'Client Copy',
  ...
  },
  {
    ...
    ja: {
      term: 'ロックエントリ（SM12）',
      reading: 'ろっくえんとり',
      definition: '同じデータを複数のユーザーが同時に更新しないよう、エンキューサーバがメモリ上のロックテーブルで保持するロック情報。SM12で一覧でき、どのユーザーがどのデータをロックしているかを確認できる。通常は処理の終了とともに解放されるが、異常終了などでロックが残ることがある。更新処理が進行中の可能性があるため、手動削除は状況を確認してから行う。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: 'メッセージサーバ',
      reading: 'めっせーじさーば',
      definition: 'ABAPシステムに1つだけ存在し、アプリケーションサーバ間の通信とログオン時の負荷分散を担うプロセス。ユーザーはメッセージサーバを通じて、SMLGで定義したログオングループの中から負荷の少ないサーバに振り分けられる。エンキューサーバとともにASCS（ABAPセントラルサービス）インスタンスに含まれ、止まるとシステム全体に影響するため高可用性構成の対象になる。',
    },
    en: {
      term: 'Message Server',
  ...
  },
  {
    ...
    ja: {
      term: 'プロジェクトビルダ',
      reading: 'ぷろじぇくとびるだ',
      definition: 'CJ20Nで起動する、プロジェクトの作成と保守をまとめて行う画面。左側のツリーでプロジェクト定義、WBS要素、ネットワーク、作業、構成品目、マイルストーンを表示し、右側で詳細を編集する。ドラッグ操作で構造を組み替えられ、テンプレートからのコピーもここで行う。個別のトランザクションを使い分けなくてもプロジェクト全体を扱えるため、PSの日常操作の中心となる。',
    },
    en: {
