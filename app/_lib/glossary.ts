...
const coreTerms: GlossaryTerm[] = [
  ...
  },
  {
    ...
    ja: {
      term: '勘定決定',
      reading: 'かんじょうけってい',
      definition: 'SDの請求処理やMMの在庫移動時に、転記先のGL勘定を自動的に決定する仕組み。条件テクニックを使用し、売上組織・勘定設定グループ・勘定キー等の組み合わせで売上勘定・消費税勘定・在庫勘定などを自動決定する。SDではKOFI（売上勘定）などの条件タイプが使用される。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '原価要素',
      reading: 'げんかようそ',
      definition: 'CO管理会計における原価・収益の分類単位。一次原価要素（Primary）はFIのGL勘定に対応し、材料費・人件費・減価償却費などを分類する。二次原価要素（Secondary）はCO内部の配賦・活動配賦・製造指図決済に使用される。S/4HANAではGL勘定と原価要素が統合されている。',
    },
    en: {
      term: 'Cost Element',
  ...
  },
  {
    ...
    ja: {
      term: 'コンサインメント',
      reading: 'こんさいんめんと',
      definition: '仕入先所有の在庫をプラントの保管場所に預かり管理する調達方式。在庫を使用（引き出し）した時点で仕入先への支払義務が発生するため、保管中はコストが発生しない。発注タイプNBに明細カテゴリK（コンサインメント）を使用し、使用時は移動タイプ411Kで自社在庫へ転換する。',
    },
    en: {
      term: 'Consignment',
  ...
  },
  {
    ...
    ja: {
      term: '条件タイプ',
      reading: 'じょうけんたいぷ',
      definition: 'SAPの条件テクニックにおける価格・割引・割増・税金などの個別要素を定義するオブジェクト。PR00（標準価格）・K004（品目割引）・MWST（消費税）などが代表的。各条件タイプにはアクセスシーケンスが割り当てられ、条件レコードの検索方法を定義する。VK11で条件レコードを登録する。',
    },
    en: {
      term: 'Condition Type',
  ...
  },
  {
    ...
    ja: {
      term: 'STO（在庫転送オーダー）',
      reading: 'えすてぃーおー',
      definition: 'Stock Transfer Order。プラント間の在庫転送を購買発注伝票で管理する特殊調達方式。発注タイプUBを使用し、送り元プラントでPGI（出荷確定）を実行して在庫を減少させ、受け入れプラントで入庫処理を行い在庫を増加させる。両プラントで在庫動作とGL転記が発生する。',
    },
    en: {
  ...
  // ─── た行 ───
  {
    ...
    ja: {
      term: '3ウェイマッチング',
      reading: 'さんうぇいまっちんぐ',
      definition: '購買発注（PO）・入庫（GR）・仕入先請求書の3者を照合してから支払を承認するMM購買プロセスの内部統制手続き。MIROで請求書照合を行う際にPO価格・数量との差異をチェックし、許容範囲超過時は請求書をブロックして手動承認を要求する。財務コンプライアンスの基本的な統制の一つ。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '購買発注',
      reading: 'こうばいはっちゅう',
      definition: '仕入先に対する正式な発注書を表すMM伝票。ME21Nで作成し、品目・数量・価格・納期・仕入先が記載される。発注タイプNB（標準）・UB（STO）・FO（枠発注）等がある。会社の購買ポリシーに応じてリリース戦略（承認フロー）が適用される。入庫（MIGO）・請求書照合（MIRO）の元伝票となる。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '製品原価計算',
      reading: 'せいひんげんかけいさん',
      definition: 'COの製品原価計算（CO-PC）コンポーネント。製品の標準原価を計算し（CK11N）、原価見積をリリース（CK24）することで品目マスタの標準価格が更新される。製造指図の実績原価と標準原価の差異が差異分析（KKS2/KKS1）で計算され、CO88決済で製品在庫や原価センタに振り替えられる。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '製造指図',
      reading: 'せいぞうさしず',
      definition: '製品を製造するための指示と実績収集の単位。CO01で作成またはCO41で計画手配から変換する。CRTD（作成済）→REL（リリース）→CNF（確認済）→TECO（技術的完了）→CLSD（クローズ）のステータスで進む。実績の材料出庫・活動数量・製造確認（CO11N）が集計され、CO88決済で実績原価が最終的に振り替えられる。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: 'MRP（所要量計画）',
      reading: 'えむあーるぴー',
      definition: 'Material Requirements Planning。需要（受注・PIR）に対してBOM展開を行い、製造および調達の所要量を自動計算するPP/MMの計画機能。MD01（全品目バッチ実行）またはMD02（品目別単体実行）で実施。結果はMD04（在庫/所要量一覧）で確認でき、計画手配・購買依頼が自動生成される。',
    },
    en: {
  ...
  // ─── わ行 ───
  {
    ...
    ja: {
      term: '仕掛品（WIP）',
      reading: 'しかかりひん',
      definition: 'Work in Process。月末時点で完了していない製造指図に投入済みのコスト。期末処理では完了していない製造指図の投入コストを仕掛品として資産計上する計算（WIP計算）を行う。TECOになった時点でWIPが解除され、標準原価との差異計算・CO88決済が実行される。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '差異分析',
      reading: 'さいぶんせき',
      definition: '製造指図の標準原価と実績原価の差異を計算・分析するCO機能。個別実行はKKS2、一括実行はKKS1を使用する。差異は価格差異（原価率の相違）・数量差異（実際投入量と標準投入量の相違）等に分類される。差異はCO88決済時にFIへ振り替えられる。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '内部指図',
      reading: 'ないぶさしず',
      definition: 'CO管理会計における特定プロジェクト・イベント・設備の原価を収集・追跡するオブジェクト。KO01で作成し、予算管理（アベイラビリティコントロール）・承認フロー・決済先（原価センタ・利益センタ・GL勘定）の設定が可能。ステータス管理により「作成済（CRTD）→リリース（REL）→技術的完了（TECO）→クローズ（CLSD）」の段階で管理される。',
    },
    en: {
      term: 'Internal Order',
  ...
  },
  {
    ...
    ja: {
      ...
      definition: 'Goods Receipt/Invoice Receipt Clearing Account。入庫（GR）から請求書照合（MIRO）完了までの間、未処理残高を一時的に保持するGL勘定。入庫時に貸方計上され、MIRO完了時に借方でクリアされる。月次処理ではMR11でGR/IR残高の整理を行う。GRのみ・MIROのみの場合は残高が残り、期末調整が必要となる。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '計画手配',
      reading: 'けいかくてはい',
      definition: 'MRP実行後に自動生成される製造・調達の計画単位。製造品目には計画手配、購買品目には購買依頼が生成される。MD04（在庫/所要量一覧）で確認でき、CO41で製造指図に変換するか、購買依頼の場合はME57/ME59Nで発注に変換する。まだ確定していない暫定計画のため、修正・削除が可能。',
    },
    en: {
      term: 'Planned Order',
  ...
  },
  {
    ...
    ja: {
      term: '買掛金',
      reading: 'かいかけきん',
      definition: '仕入先への未払債務を管理するFI補助元帳。MMの請求書照合（MIRO）時に買掛金照合勘定へ自動転記される。F110（自動支払プログラム）で支払実行し消込を行う。買掛金元帳は仕入先マスタの会社コードデータに設定された照合勘定と常に一致する。月次処理では未払残高・期日超過の確認が重要。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: 'PTP（購買から支払まで）',
      reading: 'ぴーてぃーぴー',
      definition: 'Purchase to Pay。MMの調達プロセス全体を表す業務フローの総称。購買依頼（ME51N）→購買発注（ME21N）→入庫（MIGO）→請求書照合（MIRO）→支払実行（F110）の一連の流れ。内部統制の観点では3ウェイマッチング（PO・GR・請求書の照合）が重要なコントロールポイントとなる。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '見込生産（MTS）',
      reading: 'みこみせいさん',
      definition: 'Make to Stock。受注前に需要予測（PIR）に基づいて製品を事前製造し在庫として確保する生産方式。完成品在庫から受注に対して出荷する。製造コストは製品在庫に集計され、受注明細との直接紐付けはない。食品・消耗品・標準品など安定した需要のある製品に適している。SAPではMRPタイプPDと品目マスタのPIR消費設定で実装する。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: 'MRPタイプ',
      reading: 'えむあーるぴーたいぷ',
      definition: '品目マスタのMRPデータ画面で設定するMRP計画方法コード。PD（MRP：BOM展開・正味所要量計算の標準方式）・VB（再発注点：在庫が基準量を下回ったとき計画手配を自動生成）・ND（計画なし：MRPの対象外）が代表的。品目の調達特性・需要変動パターンに応じて適切なMRPタイプを選択することが計画精度向上の鍵となる。',
    },
    en: {
      term: 'MRP Type',
