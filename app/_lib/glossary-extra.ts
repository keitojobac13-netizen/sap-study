...
export const extraTerms: GlossaryTerm[] = [
  ...
  },
  {
    ...
    ja: {
      term: '償却領域',
      reading: 'しょうきゃくりょういき',
      definition: '同じ1つの資産について、目的の異なる複数の帳簿価額を並行して管理する仕組み。会計基準上は定額法で10年、税務上は定率法で8年といった状況を、償却領域01（帳簿）と15（税務）に分けて同時に計算する。すべての領域が総勘定元帳に転記されるわけではなく、税務用は計算だけ行い申告資料の作成に使う運用が一般的。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: '減価償却キー',
      reading: 'げんかしょうきゃくきー',
      definition: '減価償却額をどう計算するかを定義するマスタ。定額法か定率法か、償却の開始タイミング、残存価額の扱いといった要素を計算方法の組み合わせとして定義する。耐用年数との組み合わせで毎期の償却額が自動計算される。耐用年数は資産マスタの償却領域ごとに設定できるため、帳簿と税務で異なる年数を使うことも可能。',
    },
    en: {
  ...
  },
  {
    ...
    ja: {
      term: 'リベート',
      reading: 'りべーと',
      definition: '一定期間の取引量に応じて事後的に支払う割戻し。取引の都度ではなく期末にまとめて決済するため、期中は引当金を積み上げていく処理を行う。リベート契約に条件レコードを登録し、請求のたびに引当額が計算される。契約期間の終了時に決済し、クレジットメモを発行する。',
    },
    en: {
      term: 'Rebate',
  ...
  },
  {
    ...
    ja: {
      term: '外注加工',
      reading: 'がいちゅうかこう',
      definition: '部品を外部の業者に支給し、加工してもらって完成品を受け取る調達形態（明細カテゴリL）。支給した部品は業者の場所にあるが所有権は自社にあり、「仕入先在庫」として管理される。完成品の入庫と同時に部品表にもとづいて部品が自動的に消費され、支払うのは加工賃のみ。仕入先在庫の残高はME2Oで定期的に確認する必要がある。',
    },
    en: {
      term: 'Subcontracting',
  ...
  },
  {
    ...
    ja: {
      term: '照合ブロック（請求書ブロック）',
      reading: 'しょうごうぶろっく',
      definition: '3ウェイマッチングで数量や金額が許容範囲を超えてずれている場合に、請求書へ自動的に設定される支払保留。会計上は債務として計上されるが支払は行われない。MRBRで一覧を確認し、内容を精査して解除するか仕入先と交渉するかを判断する。放置すると支払遅延となり仕入先との関係を損なうため、ブロック理由別に担当を決めて処理する運用が必要。',
    },
    en: {
